...
function normalizeSiteControlMeta(meta = {}, protocol = {}) {
  ...
  return {
    ...
    firstReportCreatedAt: meta.firstReportCreatedAt || protocol.firstReportCreatedAt || meta.reportCreatedAt || meta.reportFinalizedAt || meta.numberingLockedAt || meta.numbering_locked_at || "",
    reportCreatedAt: meta.reportCreatedAt || protocol.reportCreatedAt || meta.firstReportCreatedAt || meta.reportFinalizedAt || "",
    lastReportCreatedAt: meta.lastReportCreatedAt || protocol.lastReportCreatedAt || meta.reportCreatedAt || meta.firstReportCreatedAt || ""
  };
}
...
window.kaiDebugPlans = async function kaiDebugPlans() {
  ...
  return {
    ...
    lastUploadDebug: state.lastProjectPlanUploadDebug || null
  };
};
...
function siteControlNumberingLocked(protocol = state.current) {
  ...
  return !!(
    meta.numberingLocked ||
    meta.numbering_locked ||
    protocol?.numberingLocked ||
    protocol?.numbering_locked ||
    meta.firstReportCreatedAt ||
    meta.reportCreatedAt ||
    meta.lastReportCreatedAt ||
    meta.reportFinalizedAt ||
    protocol?.firstReportCreatedAt ||
    protocol?.reportCreatedAt ||
    protocol?.reportFinalizedAt
  );
}
...
function markSiteControlReportIssued(protocol = state.current) {
  if (!protocol || !isSiteControlProtocol(protocol)) return false;
  protocol.siteControl = normalizeSiteControlMeta(protocol.siteControl || {}, protocol);
  const issuedAt = new Date().toISOString();
  const wasLocked = siteControlNumberingLocked(protocol);
  ensureSiteControlItemBkNumbers(protocol);
  protocol.siteControl.numberingLocked = true;
  protocol.siteControl.numbering_locked = true;
  protocol.siteControl.numberingLockedAt = protocol.siteControl.numberingLockedAt || protocol.siteControl.numbering_locked_at || issuedAt;
  protocol.siteControl.numbering_locked_at = protocol.siteControl.numbering_locked_at || protocol.siteControl.numberingLockedAt || issuedAt;
  protocol.siteControl.numberingLockedBy = protocol.siteControl.numberingLockedBy || protocol.siteControl.numbering_locked_by || "local-user";
  protocol.siteControl.firstReportCreatedAt = protocol.siteControl.firstReportCreatedAt || protocol.siteControl.reportCreatedAt || protocol.siteControl.reportFinalizedAt || issuedAt;
  protocol.siteControl.reportCreatedAt = protocol.siteControl.reportCreatedAt || protocol.siteControl.firstReportCreatedAt;
  protocol.siteControl.lastReportCreatedAt = issuedAt;
  protocol.numberingLocked = true;
  protocol.numbering_locked = true;
  protocol.updatedAt = issuedAt;
  return !wasLocked;
}

function lockSiteControlNumbering(protocol = state.current) {
  return markSiteControlReportIssued(protocol);
}
...
function newProjectPlanUploadDebug(fileList = [], activeProjectId = "") {
  return {
    ...
    inInventory: 0,
    rendered: 0,
    files: fileList.map((file) => ({ name: file?.name || "", size: file?.size || 0, type: file?.type || guessFileType(file?.name || "") })),
    errors: [],
    lastError: ""
  };
}

function updateProjectPlanUploadDebug(patch = {}) {
  state.lastProjectPlanUploadDebug = { ...(state.lastProjectPlanUploadDebug || newProjectPlanUploadDebug()), ...patch };
  return state.lastProjectPlanUploadDebug;
}

function pushProjectPlanUploadError(step, error, extra = {}) {
  const message = error?.message || String(error || "Unbekannter Fehler");
  const debug = state.lastProjectPlanUploadDebug || newProjectPlanUploadDebug();
  debug.errors = [...(debug.errors || []), { step, message, name: error?.name || "Error", ...extra }];
  debug.lastError = `${step}: ${message}`;
  state.lastProjectPlanUploadDebug = debug;
  console.error("Projektplan-Upload fehlgeschlagen", { step, error, extra, debug });
  return debug;
}
...
function storedProjectPlanRecord(plan, file, project, protocol) {
  return {
    ...
    protocolId: protocol.id,
    planId: plan.id,
    name: plan.appPlanName || plan.title || file.name,
    title: plan.title || plan.appPlanName || file.name,
    fileName: file.name,
    filename: file.name,
    fileType: plan.type,
    mimeType: plan.type,
    ...
    appPlanName: plan.appPlanName || plan.title || "",
    category: plan.category || "Sonstiges",
    planType: plan.category || "Sonstiges",
    floor: plan.floor || "",
    component: plan.component || "",
    planNo: plan.planNo || plan.planNumber || "",
    planNumber: plan.planNumber,
...
async function importProjectPlanFiles(files, projectId = state.currentProjectId) {
  ...
  state.currentProjectId = project.id;
  updateProjectPlanUploadDebug({ activeProjectId: project.id });
  const protocol = ensureProjectPlanLibraryProtocol(project.id);
  if (!protocol) {
    pushProjectPlanUploadError("library-protocol", "Projektplanablage konnte nicht vorbereitet werden.", { projectId: project.id });
    setProjectPlanUploadState({ files: fileList, selected: fileList.length, imported: 0, saved: 0, error: "Projektplanablage konnte nicht vorbereitet werden.", message: "", busy: false });
    await renderProjectPlansView();
...
Trotzdem zus?tzlich importieren?`)) {
      ...
      plan = normalizePlanMeta({
        ...
        appPlanName: meta.appPlanName || meta.title || file.name,
        category: meta.category,
        floor: meta.floor,
        component: meta.component,
        planNumber: meta.planNumber,
      ...
      plan.blobId = plan.id;
      created += 1;
      updateProjectPlanUploadDebug({ created });

      const record = storedProjectPlanRecord(plan, file, project, protocol);
      try {
        await idbPut("plans", record);
      } catch (error) {
        pushProjectPlanUploadError("indexeddb-put", error, { fileName: file.name, planId: plan.id, errorName: error?.name || "" });
        showAppToast(`Plan konnte nicht gespeichert werden: ${error?.message || error}`, { type: "error", timeout: 8000 });
        continue;
      }
      savedCount += 1;
      updateProjectPlanUploadDebug({ saved: savedCount });

      const saved = await idbGet("plans", plan.id);
      if (!saved || !saved.blob || !saved.blob.size) {
        const error = new Error("Plan wurde gespeichert, konnte aber nicht wieder gelesen werden.");
        pushProjectPlanUploadError("indexeddb-readback", error, { fileName: file.name, planId: plan.id, hasSaved: !!saved, blobSize: saved?.blob?.size || 0 });
        showAppToast(error.message, { type: "error", timeout: 8000 });
        continue;
      }
      readBackCount += 1;
      updateProjectPlanUploadDebug({ readBack: readBackCount });

      plan.fileSize = saved.blob.size || plan.fileSize;
      protocol.plans = protocol.plans || [];
      if (!protocol.plans.some((item) => item.id === plan.id)) protocol.plans.push(plan);
      importedIds.push(plan.id);
      ...
      if (plan.type === "application/pdf") {
        try {
          ...
          console.warn("Plan-Metadaten konnten nicht automatisch gelesen werden", { error, fileName: file.name, planId: plan.id });
          plan.renderStatus = plan.renderStatus || "idle";
        }
      }

      const inventory = await buildProjectPlanInventory(project.id);
      const found = inventory.entries.some(({ plan: item }) => item.id === plan.id);
      if (!found) {
        const error = new Error("Plan wurde gespeichert, erscheint aber nicht in der Planinventur.");
        pushProjectPlanUploadError("inventory", error, { fileName: file.name, planId: plan.id, visibleCount: inventory.entries.length });
        showAppToast(error.message, { type: "error", timeout: 8000 });
        continue;
      }
      inventoryCount += 1;
      updateProjectPlanUploadDebug({ inInventory: inventoryCount });
    } catch (error) {
      pushProjectPlanUploadError("unexpected", error, { fileName: file?.name || "", planId: plan?.id || "" });
      showAppToast(`Plan konnte nicht gespeichert werden: ${error?.message || error}`, { type: "error", timeout: 9000 });
    }
  }
  ...
  const finalInventory = await buildProjectPlanInventory(project.id);
  ...
  await renderProjectPlansView();
  renderDatalists();
  showAppToast(success ? message : errorText, { type: success ? "success" : "error", timeout: success ? 4500 : 9000 });
...
const kaiVoiceCore = {
  ...
  async startDictation() {
    if (!this.isSupported()) {
      this.state.lastError = "MediaRecorder oder Mikrofonzugriff nicht verfügbar";
    ...
    try {
      ...
      showAppToast("Aufnahme gestartet", { type: "success", timeout: 1800 });
      return true;
    } catch (error) {
      this.closeStream();
      this.state.lastError = error?.message || String(error || "Unbekannter Mikrofonfehler");
      this.setStatus("error");
      showAppToast("Mikrofon konnte nicht gestartet werden: " + this.state.lastError, { type: "error", timeout: 6500 });
      return false;
    }
  },
...
function renderKaiVoiceCoreUi() {
  ...
  panel.querySelectorAll("[data-action^='voice-'], [data-kai-voice-action]").forEach((button) => {
    ...
    button.hidden = false;
    button.disabled = !supported && action !== "voice-reset";
    if (action === "voice-start") button.disabled = !supported || value.active;
    if (action === "voice-pause") {
      button.hidden = !value.active || value.paused;
      button.disabled = !supported || !value.active || value.paused;
    }
    if (action === "voice-resume") {
...
function normalizeKaiVoiceAction(action = "") {
  const map = {
    ...
    discard: "voice-reset",
    reset: "voice-reset",
    "apply-site-note": "voice-apply",
    apply: "voice-apply"
  };
  return map[action] || action;
}

async function handleKaiVoiceCoreAction(action) {
  const normalized = normalizeKaiVoiceAction(action);
  kaiVoiceCore.state.lastAction = normalized || "unbekannt";
  kaiVoiceCore.state.lastError = "";
  kaiVoiceCore.notify();
  try {
    if (normalized === "voice-start") {
      await kaiVoiceCore.start();
      return;
    }
    if (normalized === "voice-pause") {
      kaiVoiceCore.pause();
      return;
    }
    if (normalized === "voice-resume") {
      kaiVoiceCore.resume();
      return;
    }
    ...
    if (normalized === "voice-reset") {
      kaiVoiceCore.reset();
      showAppToast("Diktat verworfen", { type: "info", timeout: 1800 });
      return;
    }
    if (normalized === "voice-apply") {
      applyKaiVoiceTranscriptToOpenSiteItem();
      return;
    }
  } catch (error) {
    kaiVoiceCore.state.lastError = error?.message || String(error || "Unbekannter Fehler");
    kaiVoiceCore.notify();
    showAppToast("Voice-Aktion fehlgeschlagen: " + kaiVoiceCore.state.lastError, { type: "error", timeout: 6500 });
  }
}

document.addEventListener("click", (event) => {
  const button = event.target.closest("[data-action^='voice-'], [data-kai-voice-action]");
  if (!button) return;
  event.preventDefault();
  event.stopPropagation();
  if (button.disabled) {
    kaiVoiceCore.state.lastAction = normalizeKaiVoiceAction(button.dataset.action || button.dataset.kaiVoiceAction || "") || "deaktiviert";
    kaiVoiceCore.notify();
    return;
  }
  handleKaiVoiceCoreAction(button.dataset.action || button.dataset.kaiVoiceAction || "");
}, true);
...
function saveSiteControlForm({ persistNow = true } = {}) {
  ...
  const previousSiteControlMeta = normalizeSiteControlMeta(p.siteControl || {}, p);
  p.siteControl = {
    ...previousSiteControlMeta,
...
function renderSiteControlItems() {
  ...
  const lockHint = siteControlNumberingLocked(state.current)
    ? `<div class="info-banner small">BK-Nummern sind gesperrt. Gelöschte Punkte bleiben im Protokoll als gelöscht sichtbar.</div>`
    : "";
  list.innerHTML = `${lockHint}${items.length ? items.map(siteControlItemCard).join("") : `<div class="empty-card muted">Noch keine Feststellung. Nutze oben + Mangel, + Aufgabe, + Hinweis oder + Foto-Doku.</div>`}`;
}
...
function renderSiteControlResultSummary() {
  ...
  const locked = siteControlNumberingLocked(state.current);
  ...
  target.innerHTML = `${locked ? `<div class="info-banner small">Nummerierung gesperrt, da bereits ein Bericht erstellt wurde.</div>` : ""}<div class="site-summary-grid"><div class="mini-stat"><strong>${items.length}</strong><span>Feststellungen</span></div><div class="mini-stat"><strong>${open}</strong><span>offen</span></div><div class="mini-stat"><strong>${pins}</strong><span>Pins</span></div><div class="mini-stat"><strong>${photos}</strong><span>Fotos</span></div>${deleted ? `<div class="mini-stat"><strong>${deleted}</strong><span>gelöscht</span></div>` : ""}</div>`;
}
...
function siteControlReportPhotoGrid(cards = [], { compact = false } = {}) {
  if (!cards.length) return "";
  return `<div class="site-report-photo-grid ${compact ? "compact" : ""}">${cards.map(({ meta, item, photo, url }) => `
    <figure class="site-report-photo">
      <img src="${url}" alt="${escapeAttr(photo.name || "Foto")}">
      <figcaption><strong>${escapeHtml(meta?.title || siteControlItemContext(item))}</strong><br>${escapeHtml(photo.name || "Foto")}</figcaption>
    </figure>`).join("")}</div>`;
}
...
async function buildSiteControlReportParts() {
  ...
  for (const photo of [...(p.photos || []), ...(p.siteControl?.photos || [])]) {
    if (!photo?.id || directPhotoIds.has(photo.id)) continue;
    const url = await reportPhotoDataUrl(photo.id, { maxWidth: 1200, maxHeight: 1200, type: "image/jpeg", quality: 0.75 });
...
async function createSharePdfFile() {
  ...
  if (!blob || type !== "application/pdf") throw new Error("PDF-Datei konnte nicht erstellt werden.");
  if (typeof File !== "function") return { blob, file: null, fileName: safeName, warnings };
