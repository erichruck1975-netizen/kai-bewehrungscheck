  ...
  return [...groups.values()].map((group, index) => {
    ...
    return `
      <section class="appendix-block site-plan-appendix ${index ? "page-break" : ""}">
        <h3>Plananlage ${index + 1} – ${escapeHtml(title)} – Seite ${group.pageNumber}</h3>
...
async function buildSiteControlReportParts() {
  ...
  const photoHtml = photoCards.length ? siteControlReportPhotoGrid(photoCards, { compact: true }) : "";
  const body = `
    <div class="report-export"><main class="report-page">
      ...
      ${planReferenceHtml ? `<section class="site-plan-section"><h2>Plananlagen / Markierungen</h2>${planReferenceHtml}</section>` : ""}
